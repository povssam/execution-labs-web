...
export function GlobalStatement() {
  ...
  const scale = useTransform(scrollYProgress, [0, 1], [1.18, 1.23]);

  return (
    <section ref={sectionRef} id="studio-statement" className={`${styles.section} section-flow relative overflow-hidden`}>
      <Container className={styles.container}>
        <div className={styles.frame}>
          <motion.div className={styles.lensMedia} style={{ x: reduceMotion ? 0 : x, y: reduceMotion ? 0 : y, scale: reduceMotion ? 1.18 : scale }}>
