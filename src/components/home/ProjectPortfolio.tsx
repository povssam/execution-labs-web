...
export function ProjectPortfolio() {
  ...
  const select = useCallback((requestedIndex: number, scrollToState = true) => {
    const next = (requestedIndex + caseStudies.length) % caseStudies.length;
    ...
    window.scrollTo({ top: top + travel * (next / (caseStudies.length - 1)), behavior: reduceMotion ? "auto" : "smooth" });
  }, [reduceMotion]);

  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    const next = Math.max(0, Math.min(caseStudies.length - 1, Math.round(latest * (caseStudies.length - 1))));
    if (pointerRef.current) return;
    if (next !== activeRef.current) select(next, false);
  });
  ...
  const finishPointer = (event?: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (Math.abs(projected) > 38) select(activeRef.current + (projected < 0 ? 1 : -1));
    else setDragOffset(0);
  };

  const onKeyDown = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    if (event.key === "ArrowRight" || event.key === "ArrowDown") { event.preventDefault(); select(activeRef.current + 1); }
    if (event.key === "ArrowLeft" || event.key === "ArrowUp") { event.preventDefault(); select(activeRef.current - 1); }
    if (event.key === "Home") { event.preventDefault(); select(0); }
    if (event.key === "End") { event.preventDefault(); select(caseStudies.length - 1); }
      ...
      <div ref={sceneRef} className={styles.scrollScene}>
        <div className={styles.stickyStage}>
          <Container className={styles.container}>
            <div className={styles.instrumentation}><span>03 / Selected work</span><span>{String(activeIndex + 1).padStart(2, "0")} / {String(caseStudies.length).padStart(2, "0")}</span></div>
            <div className={styles.orbitStage} tabIndex={0} role="group" aria-label="Project portfolio. Scroll, drag, or use arrow keys to select a project. Press Enter to open the selected project." onKeyDown={onKeyDown} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishPointer} onPointerCancel={() => finishPointer()} data-dragging={dragging}>
              ...
              {caseStudies.map((study, index) => {
                const delta = relativeIndex(index, activeIndex, caseStudies.length);
                const angle = delta * 52;
                const x = Math.sin((angle * Math.PI) / 180) * 43;
                const y = -Math.cos((angle * Math.PI) / 180) * 31 + 1;
                const isActive = index === activeIndex;
                return (
                  <motion.button key={study.slug} type="button" className={styles.tile} data-active={isActive} aria-label={isActive ? `Open ${study.client} project` : `Select ${study.client} project`} onClick={(event) => { event.stopPropagation(); if (isActive) openActive(); else select(index); }} animate={{ left: `${50 + x}%`, top: `${50 + y}%`, scale: isActive ? 1.12 : Math.max(.68, 1 - Math.abs(delta) * .13), opacity: Math.max(.3, 1 - Math.abs(delta) * .23), rotate: `${delta * 5}deg`, x: dragOffset * (isActive ? .16 : .06) }} transition={reduceMotion ? { duration: .18 } : { duration: dragging ? 0 : .62, ease: [0.22, 1, 0.36, 1] }}>
              ...
              <div className={styles.centerReadout} aria-live="polite"><span>Project</span><h2 id="portfolio-title">Portfolio</h2><strong>{activeStudy.client}</strong><small>{activeStudy.category.split(",")[0]}</small></div>
